...
def get_trails():
    ...
    try:
        trails = Trail.query.filter_by(published=True).order_by(Trail.name.asc()).all()
        return jsonify({'trails': [t.to_dict() for t in trails]})
...
def create_trail():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'id': trail.id,
            'message': 'Trail kreiran uspješno'
        }), 201
...
def update_trail(trail_id):
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': 'Trail ažuriran uspješno'
        })
...
def delete_trail(trail_id):
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': 'Trail obrisan uspješno'
        })
